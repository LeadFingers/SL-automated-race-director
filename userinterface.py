...
############## replaced with the "leaderboard.py" script
...
################ no longer used replaced with the button in the leaderboard
...
def yesornowindow(exceldocname, label1 = '', label2 = '' ,pilotnames = [''],  title = ''):
    ...
    #add buttons
    button1 = tk.Button(win, text= "Ok I'll fix it", width= 40, command=
              lambda:[clickno(), win.destroy()], )
#     button2 = tk.Button(win, text= "No",width= 20, command=
#               lambda:[clickno(), win.destroy()])
    
    button1window = mycanvas.create_window(width/2, height*2/3, anchor = 'n', window = button1)
